...
const DEFAULT_CLIENT_VERSION = "24.40.0.1270";
...
const Life360APIDataMaxRetries = 1; //  Max retries to poll data from the Life360 API if the API does not throw an error.
...
async function requestOTP() {
    const options = {
        ...
        headers: {
            ...
            "ce-specversion": "1.0",
            "ce-id": uuidv4(), // dynamically generate a unique ID
            "ce-time": new Date().toISOString(),
            "X-Request-ID": uuidv4(), // another unique ID for tracking
            "X-FF-Environment": "production",
            "ce-type": "com.life360.device.signin-otp.v2",
            "ce-source": `/iOS/${clientVersion}/${deviceId}`
        },
        json: true,
        body: {}
    };

    // Use email if available; otherwise, use phone and countryCode
    if (life360_username) {
        options.body.email = life360_username;
        logger("debug", `Sending OTP request for email: ${life360_username}`);
    } else if (life360_phone && life360_countryCode) {
        options.body.countryCode = life360_countryCode;
        options.body.nationalNumber = life360_phone;
        logger("debug", `Sending OTP request for phone: ${life360_countryCode}${life360_phone}`);
    } else {
        logger("error", "OTP request failed: no valid email or phone number provided.");
        throw new Error("No valid email or phone number provided for OTP request.");
    }
    ...
    try {
        ...
        if (response.statusCode === 200) {
            ...
            logger("error", `OTP request failed with status ${response.statusCode}. Response: ${JSON.stringify(response.body)}`);
            throw new Error(`Failed OTP request with status code ${response.statusCode}`);
        }
    } catch (error) {
        // Log specific errors like 429 or other response-related information
        if (error.statusCode) {
            logger("error", `OTP request error - Status: ${error.statusCode}, Response: ${JSON.stringify(error.response.body)}`);
        } else {
            logger("error", `OTP request error: ${error.message}`);
        }
        throw error; // Re-throw error for handling up the call stack if needed
    }
}
