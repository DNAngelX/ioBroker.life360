 ...
 * Combines data from members and device locations.
 * @param {array} circles The Life360 circles to process.
 * @param {object} deviceLocations Device location data to combine with members.
 */
async function getPersons(circles, deviceLocations) {
    let persons = [];

    for (let c in circles) {
        const circle = circles[c];
        
        // Fetch members for the circle
        const membersUrl = LIFE360_URL.members.replace("{circleId}", circle.id);
        ...
        for (let m in members) {
            ...
            // Find matching device location if available
            const deviceLocation = deviceLocations.find(device => device.deviceId === member.id);

            if (index === -1) {
                // Merge member and device location data
                member.memberOf = [{ id: circle.id, name: circle.name }];
                if (deviceLocation) {
                    member.location = deviceLocation.location;  // Include location from device data
                }
                persons.push(member);
            } else {
    ...
    if (!err) {
        ...
        if (cloud_data && cloud_data.circles) {
            ...
            if (cloud_data.circles.length > 0) {
                ...
                cloud_data.persons = await getPersons(cloud_data.circles, cloud_data.deviceLocations);
